...
export default function LaunchingSoonPromo({ mode = "home" }: LaunchingSoonPromoProps) {
  ...
  return (
    <section
      className={
        isPage
          ? "overflow-hidden rounded-[24px] border border-white/70 bg-card shadow-[var(--shadow-card)]"
          : "overflow-hidden rounded-[20px] border border-white/70 bg-card shadow-[var(--shadow-card)]"
      }
    >
      <div className={`grid ${isPage ? "lg:grid-cols-[minmax(0,0.9fr)_minmax(380px,0.8fr)]" : "lg:grid-cols-[minmax(0,1fr)_420px]"}`}>
        <div className={`${isPage ? "p-6 md:p-8 lg:p-10" : "p-5 md:p-7"} bg-[linear-gradient(135deg,#fffaf3_0%,#f7efe4_58%,#eef3f6_100%)]`}>
          <div className="inline-flex items-center gap-2 rounded-full bg-[#111827] px-3 py-1.5 text-[11px] font-semibold uppercase tracking-[0.16em] text-white">
          ...
          <div className={`mt-5 grid gap-3 ${isPage ? "sm:grid-cols-3" : "sm:grid-cols-3 lg:grid-cols-1 xl:grid-cols-3"}`}>
            {perks.map((item) => (
              <div key={item.label} className="rounded-[14px] border border-white/80 bg-white/78 p-3 shadow-[0_10px_28px_rgba(16,24,39,0.05)]">
          ...
          <form onSubmit={submitNotify} className={`${isPage ? "mt-7 max-w-[560px]" : "mt-5 max-w-[520px]"} grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto]`}>
            ...
            <button
              ...
              className="flex h-12 items-center justify-center gap-2 rounded-full bg-[#111827] px-6 text-[12px] font-semibold uppercase tracking-[0.14em] text-white shadow-[0_14px_28px_rgba(17,24,39,0.16)] disabled:opacity-70"
            >
            ...
            <div className="mt-8 grid gap-3 border-t border-border-light pt-5 sm:grid-cols-2 lg:grid-cols-4">
              {trustItems.map((item) => (
                <div key={item.label} className="flex min-w-0 items-center gap-3">
                  ...
                  <span className="text-[12px] font-semibold uppercase tracking-[0.08em] text-[#101827]">{item.label}</span>
                </div>
              ))}
            </div>
          ) : (
            <Link href="/launching-soon" className="mt-5 inline-flex text-[12px] font-semibold uppercase tracking-[0.14em] text-[#101827] hover:text-fb-pink">
              View launch page
            </Link>
          )}
        </div>

        <div className={`${isPage ? "min-h-[360px]" : "min-h-[220px]"} relative overflow-hidden bg-[#111827]`}>
          <SmartImage
            src={launchImage}
          ...
          <div className="absolute left-5 top-5 flex items-center gap-2 rounded-full bg-white/88 px-3 py-2 text-[12px] font-semibold text-[#101827] shadow-[var(--shadow-sm)] backdrop-blur-md">
            <Plane className="h-4 w-4 text-[#b98745]" />
            India to Nepal
          </div>
          <div className="absolute bottom-5 left-5 right-5 rounded-[18px] border border-white/20 bg-white/88 p-4 shadow-[var(--shadow-card)] backdrop-blur-md">
            <div className="flex items-center gap-3">
              <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-[#111827] text-white">
      ...
      {isPage ? (
        <div className="grid border-t border-border-light bg-[#111827] px-5 py-4 text-white sm:grid-cols-2 lg:grid-cols-4">
          {[
